...
The load balancing component is used on the client side. The GoFrame framework provides a decoupled design, offering a highly flexible and extensible load balancing component managed by the `gsel` component. This component defines the load balancing interface and offers several built-in load balancing strategies. Developers can also implement custom load balancing strategies according to the provided interface.

## Strategy List

The `gsel` component provides several common load balancing strategies for developers to choose from:

- **LeastConnection**: The strategy with the least number of connections.
- **Random**: Random access.
- **RoundRobin**: Round-robin access.
- **Weight**: Weighted access, where the Weight parameter needs to be set during service registration.
...
Here, `gsel.SetBuilder(gsel.NewBuilderRoundRobin())` sets the global load balancing strategy to round-robin.
...
Start two servers and run the client.

**Output from server1 terminal:**
...
func main() {
    s := grpcx.Server.New() // Create a new gRPC server instance
    controller.Register(s)  // Register controllers for handling requests
    s.Run()                  // Start the gRPC server
}
```
...
func main() {
    var (
        ctx    context.Context
        conn   = grpcx.Client.MustNewGrpcClientConn("demo", grpcx.Balancer.WithRandom()) // Create a gRPC client connection using a random load balancing strategy
        client = protobuf.NewGreeterClient(conn) // Create a new Greeter client
    )

    for i := 0; i < 10; i++ {
        ctx = gctx.New() // Create a new context for each request
        res, err := client.SayHello(ctx, &protobuf.HelloRequest{Name: "World"}) // Call the SayHello method
        if err != nil {
            g.Log().Error(ctx, err) // Log any errors
            return
        }
        g.Log().Debug(ctx, "Response:", res.Message) // Log the response message
    }
}
```

- **`grpcx.Balancer.WithRandom()`**: This specifies that the client should use a random load balancing strategy when making requests.

***Running the Example***

**Start Two Servers**: Open two terminal windows and run the `server.go` file in each.
...
**Run the Client**: In another terminal, run the `client.go` file.
